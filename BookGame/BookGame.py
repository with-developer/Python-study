...
max_key2=[k for k,v in game_result.items() if max(game_result.values()) == v]

while (len(max_key2) > 1):
    
    for j in range(len(max_key2)):
        if(j+1==len(max_key2)):
            print(max_key2[j], end='')
        else:
            print(max_key2[j], end='와')
    print("가 각각 %d회로 동률로 게임을 계속합니다!" %(game_result[max_key2[j]]))
    i+=1
    game(i,max_key2)
    max_key2=[k for k,v in game_result.items() if max(game_result.values()) == v]

print("최종 승자는 %s가 %d회 승리하여 우승!" %(max_key2[0],game_result[max_key2[0]]))